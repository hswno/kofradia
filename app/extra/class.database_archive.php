...
class database_archive
{
	...
	public static function create_backup_table($table_name)
	{
		...
		\Kofradia\DB::get()->exec("CREATE TABLE $new_table_name LIKE $table_name");
	...
	public static function backup_table_export($table, $delete = null, $is_backup = null)
	{
		...
		if ($delete)
		{
			\Kofradia\DB::get()->exec("DROP TABLE $table");
		}
	...
	{
		$expire = time() - 3600*6;
		\Kofradia\DB::get()->exec("DELETE FROM users_antibot_validate WHERE time < $expire");
	}
	...
	public static function handle_table_users_views()
	{
		...
		\Kofradia\DB::get()->exec("DELETE FROM users_views WHERE time < $expire");
	}
	...
	{
		$expire = time() - 3600*6;
		\Kofradia\DB::get()->exec("DELETE FROM forms WHERE forms_created_time < $expire");
	}
	...
	public static function handle_table_log_referers()
	{
		...
		$result = \Kofradia\DB::get()->query("SELECT COUNT(*) FROM log_referers WHERE lr_time < $expire");
		...
		// overfør data
		\Kofradia\DB::get()->exec("INSERT INTO $table SELECT * FROM log_referers WHERE lr_time < $expire");
		
		// slett data
		\Kofradia\DB::get()->exec("DELETE FROM log_referers WHERE lr_time < $expire");
	...
	public static function handle_table_users_log()
	{
		...
		$result = \Kofradia\DB::get()->query("SELECT COUNT(*) FROM $table");
		$num = $result->fetchColumn(0);
	...
	public static function handle_table_sessions()
	{
		...
		\Kofradia\DB::get()->exec("DELETE FROM sessions WHERE ses_last_time < $expire");
	...
	public static function handle_table_poker()
	{
		...
		// slett det som ble flyttet over
		\Kofradia\DB::get()->exec("DELETE FROM poker WHERE poker_time_start < $expire");
	...
	public static function handle_table_lotto_vinnere()
	{
		...
		// slett det som ble flyttet over
		\Kofradia\DB::get()->exec("DELETE FROM lotto_vinnere WHERE time < $expire");
	...
	public static function handle_table_ff_bank_transactions()
	{
		...
		// slett det som ble flyttet over
		\Kofradia\DB::get()->exec("DELETE FROM ff_bank_transactions WHERE ffbt_time < $expire");
	...
	public static function handle_tables_inbox()
	{
		...
		\Kofradia\DB::get()->exec("CREATE TEMPORARY TABLE inbox_threads_temp (itt_it_id int(11) unsigned, primary key (itt_it_id))");
		...
		// ingenting å slette?
		$result = \Kofradia\DB::get()->query("SELECT COUNT(*) FROM inbox_threads_temp");
		$num_threads = $result->fetchColumn(0);
		if ($num_threads == 0)
		{
			// slett temp tabell
			\Kofradia\DB::get()->exec("DROP TEMPORARY TABLE inbox_threads_temp");
		...
		$result = \Kofradia\DB::get()->query("SELECT itt_it_id FROM inbox_threads_temp");
		...
		$result = \Kofradia\DB::get()->query("SELECT COUNT(*) FROM $table");
		$num_messages = $result->fetchColumn(0);
	...
	public static function handle_tables_forum()
	{
		...
		$result = \Kofradia\DB::get()->query("SELECT COUNT(*) FROM $table_ft");
		$num_ft = $result->fetchColumn(0);
	...
	public static function handle_ff_members($ff_id)
	{
		...
		\Kofradia\DB::get()->exec("INSERT INTO $table SELECT * FROM ff_members WHERE ffm_ff_id = $ff_id");
		...
		\Kofradia\DB::get()->exec("DELETE FROM ff_members WHERE ffm_ff_id = $ff_id AND ffm_status != ".ff_member::STATUS_DEACTIVATED);
	...
	public static function handle_ff($ff_id)
	{
		...
		// kopier over data
		\Kofradia\DB::get()->exec("INSERT INTO $table SELECT * FROM ff WHERE ff_id = $ff_id");
	...
	public static function backup_table($table_org, $where = null)
	{
		...
		// kopier over data
		\Kofradia\DB::get()->exec("INSERT INTO $table SELECT * FROM $table_org$where");
