...
// hent dagstatistikk
$result = $_base->db->query("SELECT DAY(FROM_UNIXTIME(uhi_secs_hour)) AS day, SUM(uhi_points) sum_points FROM users_hits, users_players WHERE up_u_id = $u_id AND up_id = uhi_up_id AND uhi_secs_hour >= $time_from AND uhi_secs_hour <= $time_to GROUP BY DAY(FROM_UNIXTIME(uhi_secs_hour))");
